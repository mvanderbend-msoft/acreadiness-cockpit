...
1. **Pick the target file**. Default to `AGENTS.md`. Ask only if ambiguous.
2. **Pick a strategy**. Default `flat`; recommend `nested` if the repo has > 5 top-level directories or multiple stacks.
